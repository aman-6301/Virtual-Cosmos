...
export class PlayerSprite {
  constructor(id, name, isSelf = false) {
    ...
    if (isSelf) {
      this.proximityRing = new PIXI.Graphics();
      this.proximityRing.circle(0, 0, PROXIMITY_RADIUS);
      this.proximityRing.stroke({ color: primaryColor, alpha: 0.15, width: 2 });
  ...
  setProximityState(active) {
    if (!this.proximityRing) return;
    const primaryColor = 0x8b5cf6;
    this.proximityRing.clear();
    this.proximityRing.circle(0, 0, PROXIMITY_RADIUS);
    if (active) {
      this.proximityRing.stroke({ color: primaryColor, alpha: 0.4, width: 3 });
    } else {
      this.proximityRing.stroke({ color: primaryColor, alpha: 0.15, width: 2 });
    }
  }
